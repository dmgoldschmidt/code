...
public:
  ...
  void reset(int n, ITEM fill){
    reset(n);
    for(int i = 0;i < n;i++){
      cout << format("copying %d bytes of fill to %x\n",sizeof(fill), &DATA[i]);
      DATA[i] = fill;
    }
  }
  void reset(int n){
